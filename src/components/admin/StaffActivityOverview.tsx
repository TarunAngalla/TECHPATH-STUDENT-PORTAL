...
import {
  StaffAddActivityPanel,
  type ApplicationOption,
} from "@/components/admin/StaffAddActivityPanel";
  ...
  return (
    <section className="grid gap-5" aria-labelledby="activity-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <h2 id="activity-heading" className="text-xl font-bold text-text-primary">
          {plural}
        </h2>
        <StaffAddActivityPanel kind={kind} applications={applicationOptions} />
      </div>
        ...
        <form className="grid gap-3 sm:grid-cols-[1fr_220px_auto]" method="get">
          <label className="relative">
            ...
            <Input
              name="q"
              defaultValue={query}
              placeholder={`Search candidate, company, role, or ${singular.toLowerCase()}`}
              className="pl-9"
            />
          </label>
          <Select
            name="status"
            defaultValue={status ?? ""}
            aria-label={`Filter ${plural.toLowerCase()} by status`}
          >
        ...
        {rows.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="mt-3 text-sm font-semibold text-text-primary">
              No {plural.toLowerCase()} match this view.
            </p>
            <p className="mt-1 text-xs text-text-muted">
              {applicationOptions.length > 0
                ? `Use Add ${singular.toLowerCase()} above to schedule one on an application.`
                : "Create an application first, then schedule activity here."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[1040px] text-left" aria-label={`${plural} work queue`}>
              <thead className="sticky top-0 z-[1]">
                <tr className="border-b border-border-subtle bg-surface/95 backdrop-blur-sm">
                  {[singular, "Candidate", "Schedule", "Status", "Recruiter", "Result / next action"].map(
                    (label) => (
                      <th
                        key={label}
              ...
              <tbody>
                {rows.map((row) => (
                  <tr
                    key={row.id}
                    className="border-b border-border-subtle last:border-0 align-top hover:bg-surface/40"
                  >
                    <td className="px-4 py-3">
                      <Link
                        href={`/admin/applications/${row.applicationId}`}
                        className="text-sm font-semibold text-text-primary hover:text-brand-500"
                      >
                        {row.title}
                      </Link>
                      <p className="mt-1 text-xs text-text-muted">
                    ...
                    <td className="px-4 py-3">
                      <Link
                        ...
                        className="text-xs font-semibold text-text-primary hover:text-brand-500"
                      >
                        {row.candidateName}
                      </Link>
                    ...
                    <td className="px-4 py-3 text-xs text-text-muted">
                      {row.recruiterName ?? row.recruiterEmail ?? "Unassigned"}
                    </td>
                    <td className="px-4 py-3 text-xs text-text-muted">
