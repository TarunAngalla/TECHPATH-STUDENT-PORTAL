...
import {
  StaffAddApplicationPanel,
  type CandidateOption,
} from "@/components/admin/StaffAddApplicationPanel";
  ...
  return (
    <div className="grid gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <StaffAddApplicationPanel candidates={candidates} />
      </div>
      ...
      <Card variant="glass" className="overflow-hidden bg-white">
        {applications.length === 0 ? (
          <p className="p-8 text-center text-sm text-text-muted">
            No applications yet. Use <span className="font-semibold">Add application</span> above to create one.
          </p>
        ) : (
            ...
            <table className="w-full text-left min-w-[960px]">
              <thead>
                <tr className="border-b border-border-subtle bg-surface/60">
                  {["Application", "Candidate", "Status", "Recruiter", "Next action", "Updated"].map(
                    (label) => (
                      <th key={label} className="px-4 py-3 text-[11px] font-semibold text-text-muted">
                        {label}
                      </th>
                    ),
                  )}
              ...
              <tbody>
                {applications.map((application) => (
                  <tr
                    key={application.id}
                    className="border-b border-border-subtle last:border-0 hover:bg-surface/40"
                  >
                    <td className="px-4 py-3">
                      ...
                      >
                        ...
                        <span>
                          <span className="block text-sm font-semibold text-text-primary group-hover:text-brand-500">
                            {application.companyName}
                          </span>
                          <span className="block text-xs text-text-muted">
                            {application.roleTitle} · {application.appNo}
                          </span>
                        </span>
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <StatusPill status={application.status as ApplicationStatus} />
                      <div className="text-[10px] text-text-muted mt-1">
                        {application.priority} priority
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs text-text-muted">
                      {application.recruiterName ?? application.recruiterEmail ?? "Unassigned"}
                    </td>
                    <td className="px-4 py-3 text-xs text-text-muted">
                      {application.nextAction ?? "—"}
                      {application.nextActionAt && (
                        <span className="block text-[10px] mt-1">
                          {formatDateTime(application.nextActionAt)}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs text-text-muted">
