-- The Tech Path — initial migration
-- Reflects DATABASE_SCHEMA.sql + expanded document categories
...
CREATE TABLE candidates (
  ...
  journey_stage  SMALLINT NOT NULL DEFAULT 0,
...
CREATE TABLE documents (
  ...
  category      TEXT NOT NULL CHECK (category IN (
                  'resume', 'handbook', 'stem_compliance',
                  'offer_letter', 'payslip', 'timesheet',
                  'onboarding', 'other'
                )),
...
CREATE TABLE announcements (
  ...
  title                TEXT NOT NULL,
  body                 TEXT NOT NULL,
  target_candidate_id  UUID REFERENCES candidates(id),
